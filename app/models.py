...
class User(Base):
    ...
    # Driver specific fields (for drivers only)
    id_name = Column(String, nullable=True)  # Name as on ID
    national_id = Column(String, unique=True, nullable=True)  # National ID number
    id_photo_url = Column(String, nullable=True)  # URL to ID photo
    birth_date = Column(DateTime(timezone=True), nullable=True)  # Date of birth
